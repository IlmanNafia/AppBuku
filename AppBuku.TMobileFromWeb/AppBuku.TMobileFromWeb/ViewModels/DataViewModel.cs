...
namespace AppBuku.TMobileFromWeb.ViewModels
{
    ...
    {        
        ...
        private List<ReviewBuku> daftarReview;
        public List<ReviewBuku> DaftarReview
        {
            get { return daftarReview; }
            set { SetProperty(ref daftarReview, value); }
        }
        ...
        async void PerformCmdAddTapped(Buku item)
        {
            ...
            await Shell.Current.GoToAsync(
                $"{nameof(ReviewPage)}?{nameof(ReviewViewModel.TheBukuId)}={item.Id}");
        }        
        ...
        private async void PerformCmdAdd()
        {
            int x = BukuEdit.Id;
            await Shell.Current.GoToAsync(
                $"{nameof(ReviewPage)}?{nameof(ReviewViewModel.TheBukuId)}={x}");
        }
        ...
        async void PerformReviewTapped(ReviewBuku item)
        {
            ...
            await Shell.Current.GoToAsync(
                $"{nameof(ReviewPage)}?{nameof(ReviewViewModel.Review)}={item.Id}");
        }
